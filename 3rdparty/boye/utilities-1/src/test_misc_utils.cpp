...
TestFdStreams2()
{
  ...
  const int nCount = rand() % 25000, nCharCount = nCount * sizeof(int);
  ...
  {
    int nRead = min(rand() % 50, nCharCount - nIdx);
    ...
    int nPutback = min(rand() % (fdiobuf::putbacksize + 1 - nPutbackDebt), nIdx);
