/* Adapted from rnorrexp.c, available from 
...
float nfix(void)
{
  ...
  for(;;)
  {
    ...
    { 
      do 
      {
        x=-log(UNI)*0.2904764; y=-log(UNI); /* .2904764 is 1/r */ 
      } while(y+y<x*x);
      return (hz>0)? r+x : -r-x;
    }
...
double
RandomNormalZiggurat::operator() () const throw(std::string)
{
  if (Seeded())
    return RNOR * m_dRange + m_dCenter;
  std::cerr << "Error: Ziggurat has not been seeded!\n";
  throw std::string("Error: Ziggurat has not been seeded!");
}

void
RandomNormalZiggurat::Seed(unsigned long nSeed)
{
  Seeded(true);
  zigset(nSeed);
}
