 ...
 *      - server: string.
 *   
 *    * SHUTDOWN_MASTER: Sent once from the master to itself after
 *      all slaves have been shut down, to indicate that all threads
 *      in the message passing subsystem * should terminate.  The ID
 *      of the master is "Master_node", as * defined by
 ...
 *   MessageRouter.
 * 
 *   The MessageRouter receiving thread inserts an EOF tag in the
 *   same manner as with here-documents; so that the second line of a
 *   message identifies the EOF-mark, this being a string not found
 *   anywhere in the message.  When reading, entire messages are read
 *   and queued waiting for a receiver.
 *
