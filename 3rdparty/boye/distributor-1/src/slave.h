...
protected:
  ...
  typedef std::map<std::string, TJobQueueElem> TJobMap;
  TJobMap m_currentJobs; // Keyed on job ID.

  MessagePasser m_mp;
  Feedback m_fb;

  int m_nWaitFactor, m_nNumJobsCompleted;
  double m_dTotalWorkTime;
  time_t m_fTimeJobsTaken;

  int ConnectServer(const std::string &sServer, const std::string &sSlaveProgram, const std::string &sSlaveArgs);
