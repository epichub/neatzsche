                                 ...
                                 JobQueue *pJobQueue, ThreadBarrier *pBarrier)
{
  ...
  m_fb.Info(2) << nSuccesses << " slaves created and ready (out of " << servers.size() << " requested)";
  return 0;
}
...
SlaveClient::SlaveClient()
    ...
    , m_dTotalWorkTime(0)
{
  if (Options::Instance().Option("slave-wait-factor", m_nWaitFactor))
 ...
 *******************************************************************/
int
SlaveClient::ConnectServer(const std::string &sServer, const std::string &sSlaveProgram, const std::string &sSlaveArgs)
{
  if (!m_sServer.empty())
  ...
  std::string sMasterOutputMode, sMasterInputMode;
  ...
  if (Options::Instance().Option("master-output-mode", sMasterOutputMode)
      ...
      || Options::Instance().Option("master-input-lines", nMasterInputLines))
    return m_fb.Error(E_SLAVECLIENT_CONNECT);

  if (sMasterInputMode == "SIMPLE")
  {
    std::stringstream sTmp;
    sTmp << nMasterInputLines;
    sMasterInputMode += " " + sTmp.str();
  }
...
/********************************************************************
 *   Take a set of job: Pop from the end of the queue, store ID of
...
SlaveClient::TakeJobs(bool &bJobsTaken)
{
  ...
  if (!(m_currentJobs.size() < m_pJobQueue->NumJobsPerSend()))
    return m_fb.Error(E_INTERNAL_LOGIC) << ": Slave tried to take a new job from queue, while still processing one or more old ones.";
...
SlaveClient::SendJobs()
{
  ...
  {
    ss << jit->first << "\n";
    int nNumBytes = jit->second.sJobData.size();
    ss.write(reinterpret_cast<char*>(&nNumBytes), sizeof(nNumBytes));
    ss.write(jit->second.sJobData.c_str(), nNumBytes);
  }
...
SlaveClient::ReceiveJobs(bool &bShutdown)
{
  ...
  if (sTag == "RESULTS")
  {
    ...
    for (int nRes = 0; nRes < nNumResults; nRes++)
    {
      ...
      if (ReadJobData(m_fb, ss, "BYTES", sResults)
...
SlaveClient::ProcessResults(const std::string &sJobID, const std::string &sResults, float fTime)
{
  ...
  if (m_pJobQueue->AcquireMutex(mtx))
    return m_fb.Error(E_SLAVECLIENT_PROCESSRESULTS) << ", couldn't lock job queue.";
      ...
      // Update average processing time. We do this no matter if the
      // job is in the queue or not.
  m_nNumJobsCompleted++;
