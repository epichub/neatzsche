...
double LightsimEvaluator::f(Lightsim2D *ls2d) {
  ...
  if(((int)settings->getValue("fitness_mode")) == 0) {
	...
	if(ls2d->getLightsources()->at(i)->getCellSize()>0) {
      ...
      //cout << numerator;
      if(numerator>1||numerator < 0) { cerr << "out of bounds num: "<<numerator<<" e2: " << e2 << " e: " <<  e << " hOpt: "<< hOpt << " cellNum: " << cellNum <<" emax: "<<eMax<< " LscToLsRatio: "<<LscToLsRatio<< " lsnum: "<< lsnum<<endl; exit(1); }
