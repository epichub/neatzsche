...
void Species::reproduce()
{
  ...
  if(luckymembers->size()==0&&p->getSpecies()->size()==1)
    cerr << "gen: " << p->getGeneration() << " spec size er 1 id er: "<<id<<" og member0 popchamps: "<<members->at(0)->getPopulationChampClones()<<"luckymembers size er null og age er:" << age << endl;
